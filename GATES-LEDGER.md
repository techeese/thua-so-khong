...
| `Gate 64` | 1 | nobody appears out of nowhere — a newcomer walks in along the road (five-point path, a 🚶 banner with their name, a log line, home shuttered until the door, roster "mới" + "soon" chips), selectable mid-walk, says their door line on arrival as the shutters open (remake, 2026-08-16) |
| `Gate 65` | 1 | every roof is somebody's — tapping a home names its owner and whereabouts (next season's arrival by name, never by number; a far-away owner; your own door in the girl's register), once per home per season; a neighbour indoors is a lit window not a figure — tap the door and they come out (remake, 2026-08-16) |
| `Gate 66` | 1 | the game teaches itself — the intro names TÀI × GAN × BẠN; the first three tips arrive in order, once each, tap-dismissable, never for a vet; the hụi is taught in three voices (the circle answers Cô Mai, the lesson lands in-season, the first coin says what rose and opened, the pot button names itself); "mới" until first use; "Cách chơi" / "Hụi là gì?" / glossary in both languages; tip bits survive a refresh (remake, 2026-08-16) |
| `Gate 67` | 1 | the xóm makes friends on its own — a chance acquaintance (never authored, never an existing bond, both present): one thread, one log line, a scene at the meeting place, +1 BẠN only for the already-befriended (3–6, not bloomed), nothing for the lonely; first-time lesson; connecting two who are quen says so; threads survive a refresh; the print draws them (remake, 2026-08-16) |
| `Gate 68` | 1 | the buildings are for something — errands send people indoors (13) and to the banyan (14, never for GAN ≤2 who falter at the đình instead), the roster names both; a clear season can bring a market day (three at the stall, one says so); the đình mat, the coin bowl once a round is paid, the jetty and moored sampan draw without throwing (remake, 2026-08-16) |
| `Gate 69` | 1 | a lesson is owed once per DEVICE, vet or not — with a non-empty book and no lessons key: no tip shows, yet the hụi lesson lands at season 2, the first coin's line on paying in, the pot names itself, and "where the rules live" lands after the first turn; a second vet run on the same device repeats none; the key survives `fresh()`; a fresh device that has heard everything hears nothing (v0.76) |
...
- **Gate 66 (v0.76):** setup now also clears `thua-so-khong-lessons` beside the chronicle — lessons are per-device since v0.76 and headless runs share a profile, so a fresh-run probe needs a fresh device. Assertions unchanged (`vetQuiet` still holds: tips stay off for vets).
- **Gate 21 (v0.76):** the two lift assertions now name their mechanism. Since v0.75 the xóm makes friends on its own (quen: BẠN 3–6 +1, 35 %/season), so after the circle lifts Ba 2 → 3 the same tick can quen him to 4 — read as `lifted=false` / `notPast3=false` about one run in six on committed v0.75. Old: `ba.ban===3` both times. New: `ba.ban===3`, or `ba.ban===4` **only if** the persisted `S.quen` list gained a pair naming Ba in that tick (`S._quenNow` is nulled inside the tick and cannot be read after it); ≤2 is never quen-lifted, so 4-with-quen still proves the circle did exactly +1 and never past 3. 12/12 green, one of them with quen firing and attributed.
- **Gate 69 (v0.77):** three assertions added to the vet run — `arrivalVu`/`arrivalBa` (the arrival line names whose house: *nhà anh* / *nhà chú*, and no double space anywhere in the log) and `circleNgan` (the season-1 circle answer, in Ngân's register, kept in the log; checked in its own 6.6 s step because a later `fresh()` bumps `runTok` and cancels the scene), `soonChip`/`soonNoThrow`/`soonSaid` (the dashed next-arrival seat is `tabindex=0`, a tap does not throw, and the road's line lands in the log). Old: 15 assertions. New: 21.
- **Gate 69 (v0.78):** `helpBadge`/`helpBadgeGone` — the ? button wears *mới* on a device where Sổ tay has never been opened, and drops it (bit 128 persisted) after one open. Old: 21 assertions. New: 23.
